...
While primops are typically emitted as inline code by the code generator,
non-runtime-rep polymorphic primops also have associated *wrappers*. These are
normal bindings defined in GHC.Prim, which is generated by genprimopcode. Such
wrappers are used in a few places:

 * unsaturated primop applications
 * calls made from the bytecode interpreter

The wrapper bindings are all of the form:

    eqChar# :: Char# -> Char# -> Int#
    eqChar# a1 a2 = eqChar# a1 a2
    {-# NOINLINE eqChar# #-}

The CoreToStg will see that the primop occurrence in the wrapper's RHS is a
saturated primop and will consequently generate an 'StgOpApp', which will
result in the primop's code being inlined into the RHS by the backend.

Note [Eta expanding primops]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

STG requires that primop applications (namely, 'StgOpApp's) be saturated. This
makes code generation significantly simpler since otherwise we would need to
define a calling convention for curried applications that can accommodate
representation polymorphism. In the case of unsaturated applications we either:

 * in the case of primops with fixed a runtime-rep: turn the application into a
   standard 'StgApp' of the primop's wrapper (see Note [Primop wrappers] above)
   when available

 * in the case of runtime-rep polymorphic primops: eta-expand to saturate the
   application
